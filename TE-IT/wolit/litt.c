...
int main(void)
{
	...
	//ERROR HANDLING
		...
		//TARGET CODE
			...
			for(i=0;i<target_i;i++)
			{
				for(j=0;j<lt_i;j++)
				{
					if(tf[i].address==lt[j].addr)
					{
						lit[0]=lt[j].literal[2];
						lit[1]='\0';
						l=atoi(lit);
						tf[i].mem_operand=l;
						strcpy(tf[i].opcode,"00");
			...
			for (i=0; i<lt_i; i++)
					{
						if(i==0)
						{
							pt[pool_i].index=lt[i].index;
							pool_i++;
						}
						else if (lt[i].addr!=(lt[i-1].addr+1))
						{
							pt[pool_i].index=lt[i].index;
							pool_i++;
						}
	...
	printf("\n\n-----TARGET FILE-----\n\n");
		printf("ADDRESS\tSIGN\tOPCODE\tOPERATOR\tMEM_OP\n");
			...
			printf("\n\n----LITERAL FILE-----\n\n");
		printf("Index\tLiteral name\tAddress\n");
			for(i=0;i<lt_i;i++)
						{

							printf("%d\t%s\t\t%d\n",lt[i].index,lt[i].literal,lt[i].addr);
						}
			printf("\n\n-----POOL TABLE-----\n\n");
			printf("Literal no\n");
			for(i=0;i<pool_i;i++)
